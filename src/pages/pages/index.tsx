...
import { getPages, deletePageById } from 'apiSdk/pages';
import { PageInterface } from 'interfaces/page';

type ColumnType = ColumnDef<PageInterface, unknown>;
...
export function PageListPage(props: PageListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<PageInterface>>(
    () => `/pages?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: PageInterface) => {
    if (hasAccess('page', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/pages/view/${row.id}`);
    }
  };
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/pages/view/${record.id}`} passHref legacyBehavior>
            <Button
          ...
          {hasAccess('page', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/pages/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
          ...
          {hasAccess('page', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
  ...
  return (
    <Box p={4} rounded="md" shadow="none">
      <AccessInfo entity="page" />
      <Flex justifyContent="space-between" mb={4}>
        <Text as="h1" fontSize="1.875rem" fontWeight="bold" color="base.content" {...titleProps}>
          Pages
        </Text>
        {hasAccess('page', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/pages/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
